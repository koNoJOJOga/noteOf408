...
&emsp;&emsp;&ensp;调制的方法主要有幅移键控(ASK)，频移键控(FSK)，相移键控(PSK)，**正交振幅调制(QAM)**。考试主要考察的点为正交振幅调制：在频率相同的前提下，将ASK与PSK结合起来，**形成叠加信号**。设波特率为B，采用m个相位，每个相位有n种振幅，则该QAM技术的数据传输速率R为 ${R = Blog_2(mn)}$(单位为 ${b/s}$)。
...
&emsp;&emsp;&ensp;**码元的概念**：码元是指用一个固定时长的信号波形(数字脉冲)表示一位k进制数字，代表不同离散数值的基本波形，是数字通信中数字信号的计量单位，这个时长内的信号称为k进制码元，而该时长称为**码元宽度**。
...
&emsp;&emsp;&ensp;在1.25秒内发出了5个码元，每连续的0.25秒内的电压分别是+3V，-6V，+6V，-3V，+3V。相当于我发出了 ${00，11，01，10，00}$ 这10个bit的数据。
...
&emsp;&emsp;&ensp;若某**无噪声理想信道**采用QAM调制，若该信道的最大数据传输速率不小于信噪比为30db条件下的极限数据传输速率。则该信道采用的QAM调制方案是(B)。
A. QAM-16
B. QAM-32
C. QAM-64
D. QAM-128
...
1. 虚电路可以确保**分组传输的顺序**。
2. 虚电路**建立时要进行路由选择**。
3. 虚电路**根据虚电路号VCID进行转发**。

&emsp;&emsp;&ensp;一条虚电路的**组成**：

1. 源和目的主机之间的路径(即**一系列链路和路由器**)；
2. VC号：沿着该路径的每段链路的一个号码；
3. 沿着该路径的每台路由器中的**转发表表项**。属于一条虚电路的分组将在它的首部携带一个VC号。因为一条虚电路在每条链路上可能具有不同的VC号，每台中间路由器必须用个新的VC号替代每个传输分组的VC号。**该新的VC号从转发表获得**。
...
&emsp;&emsp;&ensp;从r1的1接口收到的数据报，如果要从接口2发出，则**相当于发到VC号为22的链路上**（一个查表的过程）。
...
&emsp;&emsp;&ensp;支持VLAN的交换机允许在一个**单一的物理局域网基础设施定义多个虚拟局域网**。在一个VLAN内的主机彼此通信，交换机的端口(接口)由网络管理员划分为组。每个组构成一个VLAN，在每个VLAN中的端口形成一个**广播域**。若两个组需要相互通信，则将VLAN交换机的一个端口**与一台外部的路由器相连，并且将该端口配置为分别属于两个组**。
&emsp;&emsp;&ensp;VLAN帧由标准以太网帧与加进首部的4字节VLAN标签组成，而VLAN标签承载着该帧所属的VLAN标识符。VLAN标签由**交换机**加进以太网帧中。VLAN标签自身由一个2字节的标签协议标识符字段、一个2字节的标签控制信息字段和一个3比特优先权字段组成。

<div style=" margin: 0 auto; max-width: 60%;">
<img src="image-43.png" alt="Alt text" style="margin-top: 0; margin-bottom: 10px;" align="center">
</div>

&emsp;&emsp;&ensp;在使用VLAN交换机策略时，**不需要**在物理上区分哪个主机属于哪台交换机。**所有的交换机逻辑上属于一个大的虚拟交换机**。所有交换机上的主机在逻辑上属于这个大的虚拟交换机(VLAN交换机内部)。VLAN交换机在整体的，进行虚拟划分出广播域。
...
&emsp;&emsp;&ensp;由于802.11无线局域网不使用碰撞检测，一旦站点开始发送一个帧，它就完全地发送该帧。碰撞存在时仍发送整个数据帧将严重降低多路访问协议的性能。为了降低碰撞的可能性主要采取 ${CTS}$，${RTS}$ 信道预约机制。
...
5. 接收方接收到数据后，使用 ${CRC}$ 循环冗余校验码校验数据是否正确，如果数据正确，返回ACK确认帧；
6. 发送方**接收到ACK确认帧后，才进行下一帧的发送**，如果没有收到，则一直重传(**二进制指数退避算法**)。

&emsp;&emsp;&ensp;802.11的二进制指数**退避算法**：802.11由于是无线传播，其争用期不能采用类似于以太网 ${CSMA/CD}$ 协议所确定的两端点往返时间作为争用期。因此802.11的争用窗口采用的是**时隙机制**。
&emsp;&emsp;&ensp;比如，假定一个时隙的长度是9us，假设此时争用窗口大小为15，**意味着该窗口中包含15个时隙**，每个时隙9us。802.11未能规定争用窗口的大小，但建议最小为15，最大为1023。假定初始征用窗口 ${C_W=2^4–1=15}$，**首次争用信道时，随机退避时隙在0-15生成**。在重传时，第i次重传的 ${C_W=2^{4+i}–1}$。若是**第五次或五次以上，则 ${C_W}$ 在0-1023之间随机生成**。

<div style=" margin: 0 auto; max-width: 50%;">
<img src="image-46.png" alt="Alt text" style="margin-top: 0; margin-bottom: 10px;" align="center">
</div>

1. 如果题目中的语境考察的是**最长情况**，则隐蔽站应设置的NAV最长时间为收到主机甲发出的 ${RTS}$ 帧后，根据 ${RTS}$ 帧上的时间字段 ${SIFS+CTS+SIFS+Data+SIFS+ACK}$(假定所有的SIFS等长)。
2. 若题目考察隐蔽站接收到 ${CTS}$ 后应设置的NAV时间，则 ${NAV=SIFS+Data+SIFS+ACK}$。
3. 若题目考察**收到Data帧后**设置的NAV，则 ${NAV=Data+SIFS+ACK}$。为什么此处仍需加上Data的时间？防止出现以下情况：比如某站点A和B之间有一堵墙，A和B正在进行通信。而此时有站点C位于发送方A的近距离无阻隔的位置，A发送帧的时候，C会远比B收到的快，此时若收到Data帧后的NAV不加上一层Data时间（这是为了尽可能确保B能够在这个时间收到Data），可能会导致C提前预判信道结束使用的时间，导致冲突。

&emsp;&emsp;&ensp;802.11标准定义了比SIFS更长的时间间隔DIFS，并且规定，凡在空闲时间想发送数据的站点，必须等待时间DIFS后才能发送。其原因是为了保证所有站点发出的确认帧ACK得以优先发送。这个重要措施使得在这段DIFS时(${DATA+SIFS+ACK}$)，整个信道好像是发送方和接收方专用的，因为其他站点暂时都不能发送数据。

## LLC子层

&emsp;&emsp;&ensp;LLC功能有：

1. 向网络层提供服务
2. 建立数据链路层的逻辑连接(有连接服务的建立连接就是LLC层建立)
3. 差错控制，给帧加序号(三种滑动窗口协议的帧的序号就是LLC层添加)

&emsp;&emsp;&ensp;MAC层功能有：
...
&emsp;&emsp;&ensp;帧序号：发送方会维持一组“发送帧序号”，序号的个数取决于题目跟你说用多少bit编码。如果题目说用帧序号占用3个bit，那么也就表示帧序号理论上最多只能有8种。
...
&emsp;&emsp;&ensp;例如：发送序号字段为3bit，代表共有8个帧序号，0-7。如果下一个帧是24号帧，其对应的发送序号为 ${24 \% 8 = 0}$。
...
&emsp;&emsp;&ensp;其中W发送，代表发送窗口个数。W接收代表接收窗口的个数。n代表多少个bit编写帧序号。例如在后退N帧协议下，用3bit来编写帧序号。帧序号最多为8种。由于接收窗口固定为1，那么发送窗口只能为7个。
...
&emsp;&emsp;&ensp;发送周期：发送方从发送第一个bit开始，到接收到第一个确认帧为止。这段时间称为发送周期。
...
&emsp;&emsp;&ensp;传统网络层的主要任务是数据转发和路由选择。可以将网络层抽象地划分为数据层面和控制层面，转发是数据层面实现的功能，而路由选择是控制层面实现的功能。

<div style=" margin: 0 auto; max-width: 40%;">
<img src="image-52.png" alt="Alt text" style="margin-top: 0; margin-bottom: 10px;" align="center">
</div>

&emsp;&emsp;&ensp;路由器在控制层面生成路由表。根据路由表执行数据层面的转发任务。但这样做的一个问题，就是网络上会出现很多用于建立路由表的IP数据报。
&emsp;&emsp;&ensp;软件定义网络(SDN)，它采用集中式的控制层面和分布式的数据层面，两个层面相互分离。控制层面利用控制-数据接口对数据层面上的节点进行集中式控制，方便软件来控制网络。网络中的节点统一由分组交换机担任，相比于传统的路由器不在需要通过路由协议生成转发表以及互相交换自己的转发表。在网络的控制层面有一个逻辑上的远程控制器(可以由多个服务器组成)。远程控制器掌握各主机和整个网络的状态，为每个分组计算出最佳路径，通过Openflow协议(也可以通过其他途径)将转发表(在SDN中称为流表)下发给分组交换机。
&emsp;&emsp;&ensp;分组交换机的工作很单纯，即收到分组、查找流表(转发表)、转发分组。
